...
public class FarseerServer : IDisposable
{
  public class FarseePlayer
  {
    ...
    public HashSet<long> RegionsInView { get; set; } = [];
    public HashSet<long> RegionsLoaded { get; set; } = [];
    public Vec3i LastPos { get; set; } = null;
  }

  readonly FarseerModSystem modSystem;
  readonly ICoreServerAPI sapi;
  readonly FarseerServerConfig config;
  readonly BatchedRegionDataBuffer regionSendBuffer;

  readonly FarRegionProvider regionProvider;
  readonly Dictionary<IServerPlayer, FarseePlayer> modPlayers = [];
  ...
  public FarseerServer(FarseerModSystem modSystem, ICoreServerAPI sapi)
  {
    ...
    this.sapi = sapi;
    regionProvider = new FarRegionProvider(this.modSystem, sapi);
    regionProvider.RegionReady += LoadRegionForPlayersInView;
    regionSendBuffer = new(sapi, 8);
    ...
    try
    {
      ...
      sapi.StoreModConfig(config, "farseer-server.json");
    }
    catch (Exception e)
    {
  ...
  private void EnableForPlayer(IServerPlayer fromPlayer, FarseerEnable request)
  {
    ...
    if (modPlayers.TryGetValue(fromPlayer, out FarseePlayer player))
    {
    ...
    {
      modPlayers.Add(fromPlayer, new FarseePlayer() { ServerPlayer = fromPlayer, PlayerConfig = request.PlayerConfig });
    }
  ...
  {
    if (modPlayers.Remove(fromPlayer))
    {
  ...
  private void UpdateRegionsInView()
  {
    ...
    foreach (var player in modPlayers.Values)
    {
      ...
      var toUnload = GetRegionsNoLongerInView(regionsInViewBefore, regionsInViewNow).Where(player.RegionsLoaded.Contains).ToArray();
  ...
  {
    var relevantPlayers = modPlayers.Values
        .Where(
    ...
    {
      regionSendBuffer.Insert(regionData, [.. relevantPlayers.Select(p => p.ServerPlayer)]);
  ...
  private void PruneUnusedRegions()
  {
    ...
    foreach (var playerData in modPlayers.Values)
    {
  ...
  private static HashSet<long> GetRegionsNewInView(HashSet<long> regionsInViewBefore, HashSet<long> regionsInViewNow)
  {
    return [.. regionsInViewNow.Where(region => !regionsInViewBefore.Contains(region))];
  }

  private static HashSet<long> GetRegionsNoLongerInView(HashSet<long> regionsInViewBefore, HashSet<long> regionsInViewNow)
  {
    return [.. regionsInViewBefore.Where(region => !regionsInViewNow.Contains(region))];
  }
  ...
  {
    modPlayers.Remove(byPlayer);
  }

  public void Dispose()
  {
    regionProvider?.Dispose();
